...
module.exports = function (grunt) {
  ...
  __.functions(cordovaCli).forEach(function (name) {
    grunt.registerTask(name, function () {
      this.args.unshift(name.replace('cordova:', ''));
      // Handle URL's being split up by Grunt because of `:` characters
      if (__.contains(this.args, 'http') || __.contains(this.args, 'https')) {
        this.args = this.args.slice(0, -2).concat(__.last(this.args, 2).join(':'));
      }
      var done = this.async();
  ...
  grunt.registerTask('ripple-emulator', function () {
    grunt.config.set('watch', {
      all: {
        files: __.flatten(__.pluck(grunt.config.get('watch'), 'files')),
